...
export default class SnapConstraint extends Constraint {
  ...
  constructor(params: { orientation: Orientation; constraint: Edge }) {
    super();
  ...
  solveFor(
    ...
    if (this.constraint === "center") {
      ...
      let horizontalOffset = Math.min(
        targetRect.width / 2 - pointerRect.width * 1.5,
        0
      );
      let verticalOffset = Math.min(
        targetRect.height / 2 - pointerRect.height * 1.5,
        0
      );
      switch (this.constraint) {
        ...
        case "rightEdge":
          popoverRect.x =
            targetRect.right - horizontalOffset - popoverRect.width;
        ...
        case "bottomEdge":
          popoverRect.y =
            targetRect.bottom - verticalOffset - popoverRect.height;
